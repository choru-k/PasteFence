...
final class AppCoordinator: ObservableObject {
    ...
    private func simulatePaste() {
        ...
        guard AccessibilityHelper.hasAccessibilityPermission else {
            print("[AppCoordinator] Accessibility permission denied - cannot simulate paste")
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            ...
            if let keyDown = CGEvent(keyboardEventSource: source, virtualKey: 0x09, keyDown: true) { // 0x09 = 'v'
                ...
                print("[AppCoordinator] Simulated Cmd+V key down")
            } else {
                print("[AppCoordinator] Failed to create key down event")
            }
            ...
            if let keyUp = CGEvent(keyboardEventSource: source, virtualKey: 0x09, keyDown: false) {
                ...
                print("[AppCoordinator] Simulated Cmd+V key up")
            } else {
                print("[AppCoordinator] Failed to create key up event")
            }
        }
    }
    ...
    private func showAccessibilityPermissionAlert() {
        ...
        alert.informativeText = """
            PasteFence needs Accessibility permission to paste automatically.

            The masked text has been copied to your clipboard - press Cmd+V to paste manually.

            To enable automatic paste:
            System Settings → Privacy & Security → Accessibility → Enable PasteFence
            """
